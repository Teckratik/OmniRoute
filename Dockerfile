...
#
# --chown=node:node is applied directly on each COPY (instead of a separate
# `RUN chown -R node:node /app` after all copies) because on overlayfs-backed
# BuildKit workers, a bulk `chown -R` over an already-materialized tree forces
# a full copy-on-write duplication of every file into a new layer — nearly
# doubling the final image size for no functional benefit versus chowning at
# copy time (AI Hub Routing fork build was ~3.75GB vs upstream's ~1.96GB
# before this change; see docs/ai-hub-routing-ghcr-workflow.yml consumer).
